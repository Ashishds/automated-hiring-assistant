...
def _is_mock_mode():
    """Check if we're running in mock mode (no Supabase connection)"""
    return supabase is None


def _mock_candidate_data(candidate_id: str = None):
    """Return mock candidate data for testing"""
    return {
        "candidate_id": candidate_id or "mock-candidate-123",
        "name": "Mock Candidate",
        "email": "mock@example.com",
        "phone": "+1234567890",
        "resume_url": "https://mock-resume.com/resume.pdf",
        "resume_summary": "Mock resume summary",
        "scorecard": {"overall_score": 85, "notes": "Mock evaluation"},
...
def get_candidate_by_id(
    ...
    if _is_mock_mode():
        logger.info(f"Mock mode: Returning mock candidate data for ID: {candidate_id}")
        data = _mock_candidate_data(candidate_id)
    else:
        result = (
...
def update_candidate_by_id(candidate: Candidate) -> str:
    ...
    if _is_mock_mode():
        logger.info(f"Mock mode: Simulating update for candidate ID: {candidate.profile.candidate_id}")
        logger.info(f"Mock update data: {data}")
        return candidate.profile.candidate_id
